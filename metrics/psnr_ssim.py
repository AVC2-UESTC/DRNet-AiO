...
def psnr_torch(img1, img2):
    ...
    mse = (img1-img2).pow(2)
    mse = mse.contiguous().view([mse.shape[0],-1])
    psnr = torch.mean((-10/np.log(10))*torch.log(torch.mean(mse, dim=1)))
...
def ssim(img1, img2):
    ...
    img1 = img1.astype(np.float64)
    img2 = img2.astype(np.float64)
    kernel = cv2.getGaussianKernel(11, 1.5)           #(11,1)
    window = np.outer(kernel, kernel.transpose())     #(11,11)
